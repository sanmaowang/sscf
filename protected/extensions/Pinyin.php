...
class Pinyin
{
	...
	public function getFirstLetter($string)
	{
		...
		$m = ord($string[0]);
		$s = ord($string[1]);
		if ( $m>= 0x81 and $m <= 0xfe){
			if (isset(self::$pinyin_table[$m][$s])){
				return strtoupper(substr(self::$pinyin_table[$m][$s][0],0,1));
			}else{
				return "~";
			}
		}else{
    ...
    {
        $arr = self::str_split_unicode($string); 
        $rtn = '';
        foreach ($arr as $item)
        {
	...
	public static function translate($str){
		...
		foreach($arr as $row){
			if (is_array($row)){
				$ret .= $row[0];
			} elseif ($row >= 65 && $row <= 122) {
				$char = strtolower(chr($row));
				$ret .= $char;
			}
		}
